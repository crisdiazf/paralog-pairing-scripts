...
# N_ITERS PAIRED_ITERS [H_ADD=30] [N_HIDDEN_A=30] [N_HIDDEN_B=30] [FASTA_PATH=./PF00072_PF00512_paired.fasta] [SPLIT_SITE=111] [N_ITERS_B=N_ITERS]
...
## H_ADD sweep variant: A and B are the SAME frozen files across every sweep
# point (H_ADD only sizes the paired model's free block), so their own
# convergence peaks don't need re-tuning per H_ADD -- fixed at the values
# already established for the clipped-optimizer PF00072_PF01339 baseline
# (A=15000, B=15000) via a dedicated fine-grained probe: A peaks ~300 sweeps,
# B ~200 sweeps. The paired budget (350 sweeps) is fixed too, at that same
# baseline's tuned peak, so every H_ADD point in the sweep uses IDENTICAL
# sampling methodology -- otherwise a per-H_ADD-tuned budget would confound
# the comparison with sampling-budget effects instead of isolating the H_ADD
# effect. See run_logs/pair_results_PF01339_clipped_final_* for the probe
# this was derived from.
const GIBBS_STRIDE          = 50
const N_GIBBS_STEPS_A       = 6      # 300 sweeps
const N_GIBBS_STEPS_B       = 4      # 200 sweeps
const N_GIBBS_STEPS_PAIRED  = 7      # 350 sweeps
...
const PARAMTAG = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_N_ITERS=$(N_ITERS)_PAIRED_ITERS=$(PAIRED_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_DATA=$(DATASET_TAG)$(n_iters_b_tag())"
...
# train_potts_pfam.jl.
single_suffix_A() = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_DATA=$(DATASET_TAG)"
single_suffix_B() = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS_B)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_DATA=$(DATASET_TAG)"
paired_suffix()  = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_DATA=$(DATASET_TAG)$(n_iters_b_tag())_PAIRED_ITERS=$(PAIRED_ITERS)"
...
XA = seqs_onehot[:, 1:SPLIT_SITE, data_train_idx]
XB = seqs_onehot[:, SPLIT_SITE+1:end, data_train_idx]
...
println("XA: $(size(XA))  XB: $(size(XB))  X_AB: $(size(X_AB))  (subsampled from $(n_train) train rows)")
println("XA_val: $(size(XA_val))  XB_val: $(size(XB_val))  X_AB_val: $(size(X_AB_val))  (subsampled from $(n_val) held-out rows)")
