...
# N_ITERS PAIRED_ITERS [H_ADD=30] [N_HIDDEN_A=30] [N_HIDDEN_B=30] [FASTA_PATH=./PF00072_PF00512_paired.fasta] [SPLIT_SITE=111] [N_ITERS_B=N_ITERS] [PAIRED_TRAIN_FRAC=1.0] [USE_REWEIGHTING=true] [REG_B=0] [REG_PAIRED=0] [REG_A=0] [SEED=42] [GIBBS_STRIDE=50] [N_GIBBS_STEPS_A=600] [N_GIBBS_STEPS_B=600] [N_GIBBS_STEPS_PAIRED=600] [CONVERGENCE_LOG_EVERY=10] [N_SAMPLES_A=5000] [N_SAMPLES_B=5000] [N_SAMPLES_PAIRED=5000] [CD_STEPS=100] [BATCH_SIZE=256] [LR=1e-4] [CLIP_NORM=1.0] [PAIRED_TRAIN_N=0]
const OUTPUT_DIR = "./results_pfam"
# CD_STEPS/BATCH_SIZE/LR/CLIP_NORM: ARGS-configurable (appended after the
# sampling-only overrides, positions 1-22 untouched) so this script can
# reconstruct the checkpoint filenames of a run trained with any of these
# overridden -- see the matching comment in train_potts_pfam.jl.
const CD_STEPS   = length(ARGS) >= 23 ? parse(Int, ARGS[23]) : 100
const CLIP_NORM  = length(ARGS) >= 26 ? parse(Float64, ARGS[26]) : 1.0   # must match train_potts_pfam.jl's CLIP_NORM to reconstruct the same filenames
# Absolute-count alternative to PAIRED_TRAIN_FRAC -- see the matching comment
# in train_potts_pfam.jl. Must match its value to reconstruct the same
# paired_train_idx (and hence the same checkpoint filenames).
const PAIRED_TRAIN_N = length(ARGS) >= 27 ? parse(Int, ARGS[27]) : 0
...
const REG          = 0
const BATCH_SIZE   = length(ARGS) >= 24 ? parse(Int, ARGS[24]) : 256
const LR           = length(ARGS) >= 25 ? parse(Float32, ARGS[25]) : 1f-4
...
# K=$(CD_STEPS) and seed_tag() were missing here even though paired_suffix()
# (used for the .hdf5 filenames) already carries both -- two evaluation runs
# that differ only in CD_STEPS or SEED silently landed in the exact same
# REPORT_DIR and overwrote each other's report.md/figures with no warning.
# Confirmed happening in practice: the SEED=43-46 wn_check_paired_*.txt
# training-side checkpoints show a seed sweep was run, but no per-seed
# pair_results_* directory ever existed to match. Added in the same position
# paired_suffix() uses (K right after H_ADD, seed_tag() last) so the two stay
# easy to eyeball against each other.
const PARAMTAG = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_PAIRED_ITERS=$(PAIRED_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_SPLIT=bal_RW=$(USE_REWEIGHTING ? 1 : 0)_DATA=$(DATASET_TAG)$(n_iters_b_tag())$(paired_frac_tag())$(reg_a_tag())$(reg_b_tag())$(reg_paired_tag())$(seed_tag())"
...
for (arch, label) in ARCHS
    ...
    if isfile(path_fantasy_A_cache)
        ...
        # Re-emit the cached trace in the exact same "sweeps=... r_h=... r_v=...
        # r_hv=..." format gibbs_sample() prints live during fresh sampling
        # (see below), including the "Sampling RBM A" line itself. Driver
        # scripts that peak-find by scraping this log between "Sampling RBM A"
        # and "Sampling RBM B" markers would otherwise see nothing at all on a
        # cache hit (the trace was loaded into `conv_A` above but never
        # printed) and silently fall back to a guessed sampling budget -- this
        # went undetected across a full sweep (dozens of combos all silently
        # defaulting RBM A to the exploratory ceiling instead of its real,
        # much shorter peak) before being caught. Printing the real cached
        # trace here means a cache hit is just as peak-findable as a fresh
        # sample, so no special-casing is needed downstream.
        println("Sampling RBM A ($N_SAMPLES_A samples, $(N_GIBBS_STEPS_A*GIBBS_STRIDE) Gibbs sweeps) [from cache]...")
        for i in eachindex(conv_A.iters)
            println("    sweeps=$(conv_A.iters[i])  r_h=$(round(conv_A.r_h[i],digits=3))  r_v=$(round(conv_A.r_v[i],digits=3))  r_hv=$(round(conv_A.r_hv[i],digits=3))")
        end
    else
