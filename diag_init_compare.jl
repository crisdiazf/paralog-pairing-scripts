...
const N_TEST = 150
XA_data  = seqs_onehot[:, 1:SPLIT_SITE, train_idx[1:1000]]   # reference "data" for scoring
XA_init  = seqs_onehot[:, 1:SPLIT_SITE, val_idx[1:N_TEST]]   # held-out rows, used only as chain init
...
const DELTAS = (100, 300, 600)  # cumulative: 100, 400, 1000

println("\n=== Cold start (bias-only marginal), short budget ==="); flush(stdout)
...
println("\n=== Data-initialized start, short budget ==="); flush(stdout)
x_data = copy(XA_init)
F0d = mean(Array(free_energy(rbm_A, x_data)))
println("  initial (t=0, literal held-out data) mean F=$(round(F0d,digits=2))"); flush(stdout)
report("data-init, 0 sweeps (sanity: should be near-perfect)", x_data)
cum = 0
for d in DELTAS
    global x_data = sample_v_from_v(rbm_A, x_data; steps=d)
    global cum += d
    F = mean(Array(free_energy(rbm_A, x_data)))
    println("  cumulative sweeps=$cum  mean F=$(round(F,digits=2))"); flush(stdout)
    report("data-init, $cum sweeps", x_data)
end
