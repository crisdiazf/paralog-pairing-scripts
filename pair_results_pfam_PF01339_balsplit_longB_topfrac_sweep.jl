...
const GIBBS_STRIDE          = 25
const N_GIBBS_STEPS_A       = 40    # 1,000 sweeps -- tuned peak from fine-grained probe (r_hv=0.932, plateau ~900-1975)
const N_GIBBS_STEPS_B       = 28    # 700 sweeps -- tuned peak from fine-grained probe (r_hv=0.863, clear peak)
const N_GIBBS_STEPS_PAIRED  = 15    # 375 sweeps -- tuned peak from fine-grained probe (r_hv=0.934, peak of plateau ~150-500)
const CONVERGENCE_LOG_EVERY = 1     # log correlation trace every checkpoint (every 25 sweeps)
...
const PARAMTAG = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_N_ITERS=$(N_ITERS)_PAIRED_ITERS=$(PAIRED_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_SPLIT=bal_RW=$(USE_REWEIGHTING ? 1 : 0)_DATA=$(DATASET_TAG)$(n_iters_b_tag())$(paired_frac_tag())"
...
# train_potts_pfam.jl.
single_suffix_A() = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_SPLIT=bal_RW=$(USE_REWEIGHTING ? 1 : 0)_DATA=$(DATASET_TAG)"
single_suffix_B() = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS_B)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_SPLIT=bal_RW=$(USE_REWEIGHTING ? 1 : 0)_DATA=$(DATASET_TAG)"
paired_suffix()  = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_CLIP=$(CLIP_NORM)_SPLIT=bal_RW=$(USE_REWEIGHTING ? 1 : 0)_DATA=$(DATASET_TAG)$(n_iters_b_tag())$(paired_frac_tag())_PAIRED_ITERS=$(PAIRED_ITERS)"
...
for (arch, label) in ARCHS
    ...
    # ---- Does the paired model keep pace with the ceiling as the cutoff loosens? ----
    # Re-ranks the SAME already-computed coupling matrices at several TOP_FRAC
    # values -- no resampling needed, this is pure post-hoc ranking. If the
    # paired/ceiling ratio holds roughly constant as TOP_FRAC grows, the model
    # is capturing a stable fraction of the recoverable signal (not just an
    # artifact of the extreme-tail cutoff at 5%).
    println("\nTOP_FRAC sweep (paired / ceiling ratio at each cutoff):")
    println("  frac    Kn      paired   ceiling  indep    ratio(paired/ceiling)")
    sorted_val    = sortperm(vec(C_AB_val),    rev=true)
    sorted_train  = sortperm(vec(C_AB_train),  rev=true)
    sorted_paired = sortperm(vec(C_AB_paired), rev=true)
    sorted_indep  = sortperm(vec(C_AB_indep),  rev=true)
    for frac in (0.05, 0.10, 0.15, 0.20, 0.30, 0.50)
        Kn_f = max(1, round(Int, frac * N_VIS_A * N_VIS_B))
        tv = Set(@view sorted_val[1:Kn_f])
        op = length(intersect(tv, Set(@view sorted_paired[1:Kn_f]))) / Kn_f
        ot = length(intersect(tv, Set(@view sorted_train[1:Kn_f])))  / Kn_f
        oi = length(intersect(tv, Set(@view sorted_indep[1:Kn_f])))  / Kn_f
        ratio = ot > 0 ? op / ot : NaN
        println("  $(rpad(frac,7)) $(rpad(Kn_f,7)) $(rpad(round(op;digits=3),8)) $(rpad(round(ot;digits=3),8)) $(rpad(round(oi;digits=3),8)) $(round(ratio;digits=3))")
    end
