...
using RestrictedBoltzmannMachines: RBM, Potts, xReLU, log_pseudolikelihood, initialize!, pcd!, 
    sample_from_inputs, sample_v_from_v, free_energy, standardize
using Random
...
function callback(; rbm, iter, vm, vd, wd, kwargs...)
    ...
    if iszero(iter % 1000)
	base_name = "rbm_PF00072_k=$(k)&tt=$(iter)&l2l1_weights=$(reg)_learnrate04"
...
rbm = RBM(Potts((21, 111)), xReLU((100,)), zeros(21, 111, 100))
...
base_name = "rbm_PF00072_k=$(k)&tt=$(tt)&l2l1_weights=$(reg)_learnrate04"
dir_results = joinpath(@__DIR__, "resultados")
isdir(dir_results) || mkdir(dir_results)
