...
#
# Same ARGS convention as train_potts_pfam.jl / pair_results.jl:
# N_ITERS PAIRED_ITERS [H_ADD=30] [N_HIDDEN_A=30] [N_HIDDEN_B=30]
...
const DATASET_TAG  = "PF00072_PF00512_l$(SPLIT_SITE)"
const FASTA_PATH   = "./PF00072_PF00512_paired.fasta"

const TRAIN_FRAC = 0.7
const VAL_FRAC   = 0.15

const N_SAMPLES     = 1000    # fantasy particles drawn per model
...
const N_GIBBS_STEPS_A       = 1000
const N_GIBBS_STEPS_B       = 1000
const N_GIBBS_STEPS_PAIRED  = 1000
const CONVERGENCE_LOG_EVERY = 10    # log correlation trace every 10 checkpoints (every 1000 sweeps)

# Real alignments have far more rows than the synthetic Ising dataset
# pair_results.jl was written for (tens of thousands vs a few thousand), so
# unlike that script — which uses the *entire* training set directly for the
# "data" moment/correlation comparisons — we subsample a fixed number of rows
# for tractability. Same order of magnitude as N_SAMPLES so data vs model
# comparisons aren't skewed by sample-size differences alone.
const N_DATA_SAMPLES = 2000
...
const PARAMTAG = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_N_ITERS=$(N_ITERS)_PAIRED_ITERS=$(PAIRED_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_DATA=$(DATASET_TAG)"
...
single_suffix() = "N_HIDDEN_A=$(N_HIDDEN_A)_N_HIDDEN_B=$(N_HIDDEN_B)_H_ADD=$(H_ADD)_K=$(CD_STEPS)_N_ITERS=$(N_ITERS)_REG=$(REG)_BS=$(BATCH_SIZE)_LR=$(LR)_DATA=$(DATASET_TAG)"
...
for (arch, label) in ARCHS
    ...
    pA, pB, pP = hdf5path(arch, "rbm_A", single_suffix()), hdf5path(arch, "rbm_B", single_suffix()), hdf5path(arch, "rbm_paired", paired_suffix())
...
io = IOBuffer()
rp(x...) = println(io, x...)
rp("# Sampled-model validation (protein PF00072/PF00512, split=$(SPLIT_SITE)) — N_HIDDEN_A=$(N_HIDDEN_A), N_HIDDEN_B=$(N_HIDDEN_B), H_ADD=$(H_ADD), N_ITERS=$(N_ITERS), PAIRED_ITERS=$(PAIRED_ITERS)")
rp()
